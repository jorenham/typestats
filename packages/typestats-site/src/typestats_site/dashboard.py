...
async def _load_reports(
    ...
    for project in projects:
        ...
        versioned = sorted(
            [
                (Version(json_file.name.removesuffix(".json.gz")), json_file)
                async for json_file in project_dir.glob("*.json.gz")
            ],
    ...
    for (name, path), raw in zip(latest, raws, strict=True):
        try:
            reports.append(PackageReport.model_validate_json(gzip.decompress(raw)))
